// Package sources — внешние источники сведений о животных: русская
// Википедия и таксономическая база GBIF. Ключей ни та, ни другая не
// требуют. Пакет ничего не знает про MCP: он отдаёт разобранные данные,
// а обёртывает их в инструменты пакет mcpserver.
package sources
...
const (
	...
	DefaultUserAgent = "AITrenning_Task16/1.0 (https://github.com/AlexS8332/AITrenning_Task16)"
)

// Fetcher — HTTP GET с кэшем ответов в памяти. Клиент за одну сессию
// нередко спрашивает одно и то же дважды (поиск, потом чтение той же
// статьи), а внешние источники не любят частых запросов.
type Fetcher struct {
	HTTP      *http.Client
	UserAgent string
